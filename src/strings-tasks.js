...
function countVowels(str) {
  const a = str.match(/[aeiouyYAEIOU]/g);
  return a ? a.length : 0;
}
...
function extractNameFromTemplate(value) {
  const arr = value.match(/\b[A-Z][a-z]+ [A-Z][a-z]+\b/g);
  return arr ? arr.join('') : '';
}
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-z]/gi, (el) => rot[alphabet.indexOf(el)]);
}
